...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->category = L("Quality");
    def->tooltip = L("Reduces extrusion over the end of wall paths to soften seam endpoint dots. The amount is measured as equivalent path length, so 1 mm removes about one millimeter worth of normal extrusion over the taper distance. Set to 0 to disable.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 1;
    def->mode = comAdvanced;
    ...
    def->category = L("Quality");
    def->tooltip = L("Distance over which the visible path end taper amount is blended out before the end of a wall path.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 50;
    def->mode = comAdvanced;
